...
def attempt_download(weights):
    ...
    if len(weights) > 0 and not os.path.isfile(weights):
        d = {
            'yolov3-spp.pt':
                '1mM67oNw4fZoIOL1c8M3hHmj66d8e-ni_',  # yolov3-spp.yaml
        ...
        if not (r == 0 and os.path.exists(weights) and
                os.path.getsize(weights) > 1E6):  # weights exist and > 1MB
            os.remove(weights) if os.path.exists(
                weights) else None  # remove partial downloads
            s = "curl -L -o %s 'storage.googleapis.com/ultralytics/yolov5/ckpt/%s'" % (
                weights, file)
...
def gdrive_download(id='1n_oKgR81BJtqk75b00eAjdv03qVCQn2f', name='coco128.zip'):
    ...
    os.system(
        "curl -c ./cookie -s -L \"drive.google.com/uc?export=download&id=%s\" > /dev/null"
        % id)
