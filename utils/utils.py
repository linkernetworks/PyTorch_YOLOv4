...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    lcls, lbox, lobj = ft([0]).to(device), ft([0]).to(device), ft([0
                                                                  ]).to(device)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            lbox += (1.0 -
                     giou).sum() if red == 'sum' else (1.0 -
                                                       giou).mean()  # giou loss

            # Obj
            tobj[b, a, gj, gi] = (
                1.0 - model.gr) + model.gr * giou.detach().clamp(0).type(
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            if style == 'rect2':
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T
                a, t = torch.cat((a, a[j], a[k]), 0), torch.cat((t, t[j], t[k]),
                                                                0)
...
def create_pretrained(f='weights/best.pt', s='weights/pretrained.pt'
    ...
    torch.save(x, s)
    print('%s saved as pretrained checkpoint %s, %.1fMB' %
          (f, s, os.path.getsize(s) / 1E6))
...
def plot_study_txt(f='study.txt',
    ...
    for f in [
            'coco_study/study_coco_yolov5%s.txt' % x
            for x in ['s', 'm', 'l', 'x']
    ]:
        y = np.loadtxt(f,
                       dtype=np.float32,
